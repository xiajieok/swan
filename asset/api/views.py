...
class AssetList(Resource):
		...
		try:
			...
			for i in asset:
				res[i.id] = {'hostname': i.hostname, 'type': i.type, 'sn': i.sn, 'ip': i.ip,
				             'model': i.model, "cpu_cores": i.cpu_cores, "cpu_model": i.cpu_model, "cpu_num": i.cpu_num,
	...
	def post(self):
		...
		res = models.Asset(id=new_id, hostname=json_data['hostname'], sn=json_data['sn'], type=json_data['type'],
		                   model=json_data['model'], cpu_cores=json_data['cpu_cores'], cpu_model=json_data['cpu_model'],
...
class Asset(Resource):
	def get(self, asset_id):
		...
		res[asset.id] = {'hostname': asset.hostname, 'type': asset.type, 'sn': asset.sn, 'model': asset.model,
		                 'ip': asset.ip, "cpu_cores": asset.cpu_cores, "cpu_model": asset.cpu_model,
